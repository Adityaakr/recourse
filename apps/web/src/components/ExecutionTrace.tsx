...
export function ExecutionTrace({ job, timeline }: { job: Job | null; timeline: TimelineEvent[] }) {
  if (!job) return <Panel title="Execution trace"><Empty>The step-by-step trace appears once a job is live.</Empty></Panel>;

  const events = timeline.filter((e) => e.jobId === job.id).sort((a, b) => a.seq - b.seq);

  return (
    <Panel title="Execution trace" hint={`Job #${job.id} · ${events.length} steps`}>
      {events.length === 0 ? (
            ...
            <li key={e.seq} className="animate-rise relative">
              <span
                className={`absolute -left-6 top-1 h-3.5 w-3.5 rounded-full border-2 border-background ${
                  e.lane === "injected" ? "bg-lane-injected" : e.lane === "l1" ? "bg-lane-l1" : "bg-muted-foreground"
                }`}
                aria-hidden
              />
